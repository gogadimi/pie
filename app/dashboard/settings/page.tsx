...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={field.key} className="flex items-center justify-between">
              ...
              <input
                type="number"
                value={(rails as any)[field.key]}
                onChange={(e) => setRails({ ...rails, [field.key]: Number(e.target.value) })}
                className="w-24 px-3 py-2 text-right border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              />
            </div>
          ))}
          <div className="flex items-center justify-between py-2">
            ...
            <button
              onClick={() => setRails({ ...rails, requireApproval: !rails.requireApproval })}
              className={`relative w-12 h-6 rounded-full transition-colors ${
                rails.requireApproval ? 'bg-indigo-600' : 'bg-slate-300'
              }`}
            >
              <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${
                rails.requireApproval ? 'translate-x-6' : ''
              }`} />
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        ...
        <div className="space-y-3">
          {['Email', 'Slack', 'Telegram', 'Webhook'].map((channel) => (
            <div key={channel} className="flex items-center justify-between p-3 rounded-lg bg-slate-50">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2 mb-4">
          <Globe className="w-5 h-5 text-indigo-500" /> Integrations
        </h2>
        <div className="space-y-3">
          {[
      ...
      <button
        onClick={handleSave}
        className="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium text-sm transition-colors"
      >
